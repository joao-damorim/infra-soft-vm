...
def execute():
    if request.method == 'POST':
        ...
        if so == "Windows" or so == "windows":
             shell.exec('VBoxManage hostonlyif remove vboxnet0 && VBoxManage hostonlyif create && VBoxManage hostonlyif ipconfig vboxnet0 --ip 192.168.56.1 && VBoxManage modifyvm Windows --hostonlyadapter1 vboxnet0 && VBoxManage modifyvm Windows --nic1 hostonly && VBoxManage dhcpserver modify --ifname vboxnet0 --ip 192.168.56.1 --netmask 255.255.255.0 --lowerip '+str(ip)+' --upperip '+str(ip)+' && VBoxManage modifyvm Windows --cpus '+str(cpus)+' --memory '+str(memoria)+' --vram 32 --boot1 dvd && VBoxManage clonevm '+str(so)+' --name '+str(nome_clone)+' --register && VBoxManage modifyvm '+str(nome_clone)+' --nic1 hostonly && VBoxManage startvm '+str(nome_clone))
